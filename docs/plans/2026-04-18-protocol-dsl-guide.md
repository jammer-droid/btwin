...
- protocol은 phase/actions/procedure/decided_by/outcomes를 통해 guard 판단에 필요한 힌트를 제공

장기 목표:

- protocol이 named protocol guard set을 참조할 수 있게 하기
...
현재 protocol author는 guard를 직접 선언하지 않는다.

현재 guard 관련 선언은 phase의 다른 필드가 간접적으로 제공한다.
...
장기적으로는 protocol이 protocol-level guard reference를 적을 수 있어야 한다.
...
- 위 예시는 목표 방향 예시지, 현재 지원 schema는 아니다
...
- named protocol guard reference
- guard set declaration
...
현재 protocol DSL의 중심은 `phase + procedure + transition`이며, `gate`는 현재 `transition`으로 표현된다. `guard`는 아직 runtime이 소유하는 baseline policy지만, 장기적으로는 protocol author가 named guard reference를 선언할 수 있는 방향으로 확장한다.
