...
run_btwin agent create alice --provider codex --role reviewer --model gpt-5 >/dev/null
...
APPLY_ONE_MESSAGES_JSON="$(BTWIN_ATTACHED_SMOKE_THREAD_ID="${THREAD_ID}" run_python - <<'PY'
import os
import urllib.request

base = os.environ["BTWIN_API_URL"]
thread_id = os.environ["BTWIN_ATTACHED_SMOKE_THREAD_ID"]
with urllib.request.urlopen(f"{base}/api/threads/{thread_id}/messages") as response:
    print(response.read().decode("utf-8"))
PY
)"
...
JSON_PAYLOAD_APPLY_ONE_MESSAGES="${APPLY_ONE_MESSAGES_JSON}" \
...
apply_one_messages = json.loads(os.environ["JSON_PAYLOAD_APPLY_ONE_MESSAGES"])
...
assert apply_one["dispatch"]["dispatched"] is True, apply_one
assert apply_one["dispatch"]["target_agents"] == ["alice"], apply_one
assert any(
    isinstance(message, dict)
    and message.get("msg_type") == "dispatch"
    and message.get("from") == "btwin"
    and message.get("target_agents") == ["alice"]
    for message in apply_one_messages
), apply_one_messages
...
assert "mode=runtime-attached" in hud, hud
...
assert "alice contributed attached" in hud, hud
...
assert inbox["pending_message_count"] == 2, inbox
...
print(f"- apply-next dispatch: dispatched={apply_one['dispatch']['dispatched']} target={','.join(apply_one['dispatch']['target_agents'])}")
...
print(f"- hud mode label visible: {'mode=runtime-attached' in hud}")
...
print(f"- hud agent session visible: {'Agent Sessions' in hud and 'alice contributed attached' in hud}")
